...
# install.packages(c("shiny", "ggplot2", "dplyr", "plotly", "forestplot", "DT", "readr"))
...
server <- function(input, output, session) {
  ...
  output$var_selectors <- renderUI({
    ...
    tagList(
      ...
      selectInput("x_var", "X Variable:", 
                  choices = all_vars, 
                  selected = all_vars[1]),
      selectInput("y_var", "Y Variable (for scatter/line):", 
  ...
  output$metadata_info <- renderPrint({
    ...
    sapply(names(missing), function(col) {
      cat(sprintf("  %s: %d (%.2f%%)\n", col, missing[col], 
  ...
  output$main_plot <- renderPlotly({
    ...
    if (input$chart_type == "forest") {
      return(NULL)  # Forest plot rendered separately
    }
    
    # Handle grouping
    if (input$group_var != "None") {
      color_var <- input$group_var
    } else {
      color_var <- NULL
    }
    ...
    p <- switch(input$chart_type,
      ...
      "line" = {
        req(input$y_var)
        if (!is.null(color_var)) {
          ggplot(df, aes_string(x = input$x_var, y = input$y_var, color = color_var, group = color_var)) +
            geom_line(size = 1) +
            geom_point() +
            theme_minimal() +
            labs(title = paste("Line Chart:", input$y_var, "vs", input$x_var))
        } else {
          ggplot(df, aes_string(x = input$x_var, y = input$y_var)) +
        ...
        if (ncol(numeric_data) > 1) {
          ...
          ggplot() + 
            annotate("text", x = 0.5, y = 0.5, 
                     label = "Not enough numeric variables for correlation", 
                     size = 6) +
            theme_void()
        }
      }
  ...
  output$forest_plot <- renderPlot({
    req(data(), input$chart_type, input$forest_var)
